<template>
  <div class="library">
    <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md min-h-[70vh]">
      ...
      <!-- Upload Progress -->
      <div v-if="uploadStatus" class="mb-6 p-4 rounded-lg" 
        :class="{
        ...
        <div class="flex items-center">
          <span v-if="uploadStatus.type === 'loading'" class="animate-spin rounded-full h-5 w-5 border-t-2 border-b-2 border-blue-600 mr-2"></span>
          <span v-else-if="uploadStatus.type === 'success'" class="text-green-600 text-xl mr-2">✓</span>
          <span v-else-if="uploadStatus.type === 'error'" class="text-red-600 text-xl mr-2">✗</span>
          <span v-else-if="uploadStatus.type === 'warning'" class="text-yellow-600 text-xl mr-2">⚠️</span>
          <p :class="{
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="aspect-w-2 aspect-h-3 bg-gray-200 mb-3 rounded relative">
            ...
            <div v-if="book.status === 'unavailable'" class="absolute inset-0 bg-gray-600 bg-opacity-70 flex items-center justify-center text-white p-2 text-center">
              <div>
            ...
            <div v-else-if="book.status === 'corrupt'" class="absolute inset-0 bg-red-600 bg-opacity-70 flex items-center justify-center text-white p-2 text-center">
              <div>
                <span class="text-xl">⚠️</span>
          ...
          <div class="mt-3 flex space-x-2">
            ...
            <template v-if="book.status === 'unavailable' || book.status === 'corrupt'">
              <button 
...
import { restoreUploadedBooks, checkStorageAvailability } from '../utils/bookStorage'
import { storeWithChunking, clearChunks } from '../utils/storageUtils'
import { storeBookData, isIndexedDBAvailable, deleteBookData } from '../utils/indexedDbStorage'
...
const confirmDeleteBook = async (book) => {
  ...
  const isConfirmed = confirm(`Are you sure you want to delete "${bookTitle}" from your library?\n\nThis action cannot be undone.`);
  ...
  if (isConfirmed) {
    ...
    try {
      ...
      if (!isRemoved) {
        console.warn(`Book ${bookId} may not have been completely removed. Forcing another cleanup...`);
    ...
    // All books are treated as user-uploaded now
    {
      ...
      if (isIndexedDBAvailable()) {
        ...
        try {
          ...
          if (deleted) {
            console.log(`Successfully removed book data for ${book.id} from IndexedDB`);
          } else {
            console.log(`No data found for ${book.id} in IndexedDB or deletion not confirmed`);
          }
        } catch (e) {
...
onMounted(async () => {
  ...
  try {
    ...
    updatedBooks.forEach(book => {
      ...
      } else if (book.url && book.url !== libraryStore.getBookById(book.id).url) {
...
const handleFileUpload = async (event) => {
  ...
  try {
    ...
    const fileBlob = new Blob([await file.arrayBuffer()], { type: 'application/epub+zip' })
    ...
    try {
      ...
      console.log(`File converted to base64, length: ${base64Data.length} characters`);
      ...
      if (fileSizeInMB > 2 && isIndexedDBAvailable()) {
        console.log(`Using IndexedDB storage for large file (${fileSizeInMB.toFixed(2)} MB)`);
      ...
      if (stored) {
        ...
        uploadStatus.value = {
          type: 'success',
          message: `"${title}" (${fileSizeInMB.toFixed(2)} MB) has been added to your library`
        };
      } else {
