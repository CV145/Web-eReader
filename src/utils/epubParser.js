...
export class EpubParser {
  ...
  async loadFile(input) {
    try {
      ...
      console.log('EpubParser: EPUB data received, buffer size:', arrayBuffer.byteLength, 'bytes');
      ...
      try {
        ...
        this.zip = zipData;
        console.log('EpubParser: ZIP loaded successfully. Files found:', Object.keys(zipData.files).length);
      } catch (zipError) {
  ...
  async parseContainer() {
    try {
      ...
      fileKeys.forEach(file => {
        console.log(`  - ${file}${this.zip.files[file].dir ? ' (directory)' : ''}`);
      });
      
      // Check if container.xml exists
      const containerFile = this.zip.file('META-INF/container.xml');
      if (!containerFile) {
        console.error('EpubParser: container.xml not found in META-INF directory');
        ...
        throw new Error('container.xml not found and could not locate OPF file');
      }
      
      // Parse container.xml
      const containerXml = await containerFile.async('text');
      console.log('EpubParser: container.xml content:', containerXml.substring(0, 200) + '...');
      ...
      if (parserError) {
        console.error('EpubParser: XML parsing error:', parserError.textContent);
  ...
  async parseOPF() {
    try {
      ...
      if (metadataEl) {
        this.metadata.title = this.getElementText(metadataEl, 'title') || 'Untitled';
        this.metadata.creator = this.getElementText(metadataEl, 'creator') || 'Unknown';
        this.metadata.language = this.getElementText(metadataEl, 'language') || 'en';
        this.metadata.publisher = this.getElementText(metadataEl, 'publisher') || '';
        this.metadata.identifier = this.getElementText(metadataEl, 'identifier') || '';
      }
  ...
  async parseTOC() {
    try {
      ...
      const tocItem = Object.values(this.manifest).find(item => 
      ...
      const parseNavPoint = (navPoint, level = 0) => {
        ...
        const childNavPoints = Array.from(navPoint.children).filter(el => el.tagName === 'navPoint');
      ...
      navPoints.forEach(navPoint => {
        ...
        if (!navPoint.parentElement || navPoint.parentElement.tagName !== 'navPoint') {
          const item = parseNavPoint(navPoint);
          if (item) this.toc.push(item);
        }
      });
      
      console.log('EpubParser: Parsed TOC successfully, entries:', this.toc.length);
    } catch (error) {
  ...
  async getFileContent(href) {
    try {
      ...
      console.error(`EpubParser: Error getting file content for ${href}:`, error);
      throw error;
    }
  }
